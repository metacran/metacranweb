...
async function do_version_badge(res, next, pkg, query) {
    // this does not fail for non-existing packages currently
    const url = urls.crandb + '/-/desc?keys=["' + pkg + '"]';
    const body = await ky.get(url).json();
    const message = !!body[pkg] ? body[pkg]["version"] : "not published";
    const svg = make_badge(res, "CRAN", message, query);
    res.set(200);
    res.send(svg);
}

async function do_lastrelease_badge(res, next, pkg, query, type) {
