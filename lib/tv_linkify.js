...
function tv_linkify(tv) {
    ...
    if (tv.info)  { tv.info = do_replace(tv.info); }
    if (tv.links) { tv.links = tv.links.map(do_replace2); }
...
function do_replace(entry) {
	...
	.replace(/<bioc>(.*?)<\/bioc>/g,
		 '<a href="https://bioconductor.org/packages/release/bioc/html/$1.html">$1</a>')
	.replace(/<ohat>(.*?)<\/ohat>/g,
		 '<a href="https://www.omegahat.org/$1">$1</a>')
...
function do_replace2(entry) {
	...
	.replace(/<bioc>(.*?)<\/bioc>/g,
		 'Bioconductor package: <a href="https://bioconductor.org/packages/release/bioc/html/$1.html">$1</a>')
	.replace(/<ohat>(.*?)<\/ohat>/g,
		 'Omegahat package: <a href="https://www.omegahat.org/$1">$1</a>')
	.replace(/<rforge>(.*?)<\/rforge>/g,
